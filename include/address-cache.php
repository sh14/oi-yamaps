...
function set_address_cache( $user_key, $yandex_data, $expire = /*WEEK_IN_SECONDS*/ 30 ) {
	global $wpdb;
	$table        = $wpdb->prefix . 'cache';
	$user_key = esc_sql( $user_key );
	$user_key = trim( $user_key );
	...
	$query  = $wpdb->prepare( "INSERT INTO {$table} SET `user_key` = %s, `yandex_data` = %s, `expire` = %d", $user_key, $yandex_data, $expire );
	$wpdb->query( $query );
